...
function hypergeometricP(picks: number, hits: number): number {
  return (
    (combination(20, hits) * combination(60, picks - hits)) /
    combination(80, picks)
  );
}
    ...
    <div className="relative min-h-screen bg-[#0e1117] flex flex-col items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-[360px] flex flex-col items-center gap-6">
        ...
        <div className="flex items-center gap-4">
          <span
            className={`text-sm font-bold tracking-wide transition-colors ${selected === "demo" ? "text-white" : "text-slate-500"}`}
          >
            DEMO
          </span>
          <button
          ...
          <span
            className={`text-sm font-bold tracking-wide transition-colors ${selected === "real" ? "text-white" : "text-slate-500"}`}
          >
            REAL
          </span>
        </div>
          ...
          {selected === "demo" ? (
            <>
              <p className="text-sm text-slate-200 font-medium">
                Play with virtual credits — no real money involved
              </p>
              <p className="text-xs text-purple-400 font-semibold">
                10,000 demo credits to start
              </p>
            </>
          ) : (
            <>
              <p className="text-sm text-slate-200 font-medium">
                Play with real USDT from your Keno wallet
              </p>
              <p className="text-xs text-amber-400 font-semibold">
                Top up from your main wallet anytime
              </p>
            </>
          )}
        </div>
        ...
        <div>
          ...
          {type === "withdraw" && (
            <p className="text-xs text-muted-foreground mt-1">
              Keno balance: {maxAmount.toFixed(2)} USDT
            </p>
          )}
          {type === "topup" && (
            <p className="text-xs text-muted-foreground mt-1">
...
function PaytableSheet({
  ...
  const lookup = new Map(
    paytable.map((row) => [`${row.picks}:${row.hits}`, row.multiplier]),
  );
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-[#182021] rounded-2xl border border-white/10 p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-bold text-white">
              Payouts & winning combinations
            </h3>
            <p className="mt-1 text-xs text-slate-400">
              Multiplier × your bet amount · highlighted column is Pick{" "}
              {picksCount || "—"}
            </p>
          </div>
          ...
          <table className="w-full min-w-[640px] border-collapse text-center text-[11px]">
            <thead>
              <tr className="bg-white/5 text-slate-300">
                <th className="border-b border-r border-white/10 px-2 py-2 text-left font-bold">
                  Hits \ Picks
                </th>
                {Array.from({ length: 10 }, (_, i) => i + 1).map((picks) => (
                  <th
                    key={picks}
                    className={`border-b border-white/10 px-2 py-2 font-black ${picks === picksCount ? "bg-cyan-400/15 text-cyan-300" : ""}`}
                  >
                    {picks}
                  </th>
                ))}
            ...
            <tbody>
              {Array.from({ length: 11 }, (_, hits) => (
                ...
                >
                  <th className="border-r border-white/10 bg-white/[0.03] px-2 py-2 text-left font-bold text-slate-400">
                    {hits}
                  </th>
                  {Array.from({ length: 10 }, (_, i) => i + 1).map((picks) => {
                    ...
                    return (
                      <td
                        key={picks}
                        className={`px-2 py-2 font-bold ${picks === picksCount ? "bg-cyan-400/[0.08]" : ""} ${multiplier > 0 ? "text-emerald-300" : "text-slate-600"}`}
                      >
                        {valid && multiplier > 0
        ...
        {picksCount > 0 && (
          <p className="mt-3 text-xs text-slate-400">
            Pick {picksCount} pays when you match one of the winning ball
            combinations shown in the {picksCount} column. For example, a{" "}
            {picksCount}-hit result pays the multiplier on that row.
          </p>
        )}
      </div>
    </div>
...
function FairnessSheet({
  ...
  const canVerify = Boolean(
    seed && roundState?.serverHash && roundState.seedTimestamp,
  );
  ...
  async function verifyCommitment() {
    if (
      !canVerify ||
      !seed ||
      !roundState?.serverHash ||
      !roundState.seedTimestamp
    )
      return;
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 bg-[#182021] p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Verify that the winning ball combination was committed before
                betting.
              </p>
            </div>
          </div>
          <button
            ...
            aria-label="Close fairness information"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="mt-5 space-y-4 text-sm text-slate-300">
          <p>
            Before each round, XendrX Keno creates a secret server seed and
            publishes its SHA-256 hash. The winning combination is generated
            from that committed seed, the round ID, and the draw timestamp. The
            seed is revealed after the draw so anyone can check it.
          </p>
          <div className="rounded-xl border border-white/10 bg-black/20 p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-1 font-mono text-xs text-white">
                  {roundState?.drawTimestamp
                    ? new Date(roundState.drawTimestamp).toISOString()
                    : "Shown after draw"}
                </p>
              </div>
            </div>
            <div className="mt-3">
              ...
              <div className="mt-1 flex items-start gap-2">
                <code className="min-w-0 break-all text-[10px] text-cyan-300">
                  {roundState?.serverHash ?? "Not available"}
                </code>
                {roundState?.serverHash && (
                  <button
                    onClick={copyHash}
                    className="shrink-0 rounded p-1 text-slate-400 hover:bg-white/10 hover:text-white"
                    aria-label="Copy server hash"
                  >
                    {copied ? (
            ...
            <div className="mt-3">
              ...
              <code className="mt-1 block break-all text-[10px] text-emerald-300">
                {seed ?? "Hidden until the round is drawn"}
              </code>
            </div>
          </div>
          ...
          {drawn.length > 0 && (
            <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-4">
              <p className="text-[10px] font-black uppercase tracking-wider text-emerald-300">
                Winning ball combination · {drawn.length} balls
              </p>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {drawn.map((number, index) => (
                  <span
                    key={`${number}-${index}`}
                    className="flex h-7 min-w-7 items-center justify-center rounded-full bg-emerald-400 px-1.5 text-[10px] font-black text-[#10221c]"
                  >
                    {number}
                  </span>
                ))}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={!canVerify || verification === "checking"}
              className="inline-flex items-center gap-2 rounded-lg bg-emerald-500 px-3 py-2 text-xs font-bold text-[#10221c] transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <ShieldCheck className="h-3.5 w-3.5" />{" "}
              {verification === "checking" ? "Checking…" : "Verify commitment"}
            </button>
            {verification === "valid" && (
              <span className="text-xs font-bold text-emerald-300">
                Verified: the revealed seed matches the published hash.
              </span>
            )}
            {verification === "invalid" && (
              <span className="text-xs font-bold text-red-300">
                Could not verify this commitment. Please try again after the
                draw is complete.
              </span>
            )}
          </div>
          <p className="text-xs leading-relaxed text-slate-500">
            The commitment check verifies SHA-256(seed | round ID | seed
            timestamp). The draw engine then uses a deterministic HMAC-SHA-256
            shuffle to select 20 unique numbers from 1–80. During betting, only
            the hash is visible; the seed stays hidden.
          </p>
        </div>
      </div>
    </div>
  ...
  function Ball({ num, globalIdx }: { num: number | null; globalIdx: number }) {
    ...
    return (
      ...
      >
        ...
        {/* Shine gloss */}
        <span
          className="pointer-events-none absolute inset-0 rounded-full bg-gradient-to-br from-white/50 to-transparent opacity-60"
          style={{ clipPath: "ellipse(55% 45% at 38% 28%)" }}
        />
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 space-y-1.5 px-1">
      <div className="flex items-center gap-1 sm:gap-1.5">
        {row1.map((num, i) => (
          <Ball key={i} num={num} globalIdx={i} />
        ))}
      </div>
      <div className="flex items-center gap-1 sm:gap-1.5">
        {row2.map((num, i) => (
          <Ball key={i + 10} num={num} globalIdx={i + 10} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[9996] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm bg-card rounded-2xl border border-border p-5 text-center space-y-3 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div
          className={`w-14 h-14 rounded-full mx-auto flex items-center justify-center ${won ? "bg-emerald-600/20" : "bg-muted/30"}`}
        >
          {won ? (
        ...
        <div>
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-0.5">
            {won ? "Round complete!" : "No wins this round"}
          </p>
          <p className="text-2xl font-black text-emerald-400">
            {totalPayout > 0 ? `+${totalPayout.toFixed(2)}` : "0.00"}{" "}
            <span className="text-base font-medium text-emerald-400/70">
              USDT
            </span>
          </p>
          <p className="text-xs text-slate-500 mt-0.5">
            {tickets.length} ticket{tickets.length !== 1 ? "s" : ""} played
          </p>
        </div>
        ...
        {ticket && (
          <div className="rounded-xl border border-white/10 bg-[#1b2324] p-3 text-left space-y-2">
            <div className="flex justify-between text-xs">
              ...
              >
                {ticket.hitCount} hit{ticket.hitCount !== 1 ? "s" : ""} ·{" "}
                {ticket.multiplier}× ·{" "}
                {ticket.payout > 0 ? `+$${ticket.payout.toFixed(2)}` : "no win"}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-1.5 text-center">
              <div className="rounded-lg bg-white/5 px-1.5 py-2">
                ...
                <p className="mt-1 text-sm font-black text-white">
                  {ticket.hitCount}/{ticket.picks.length}
                </p>
              </div>
              <div className="rounded-lg bg-white/5 px-1.5 py-2">
                <p className="text-[9px] font-black uppercase tracking-wider text-slate-500">
                  ...
                  <span
                    ...
                    className={`flex h-7 min-w-7 items-center justify-center rounded-full px-1.5 text-[10px] font-black ${
                      drawn.includes(number)
                        ? "bg-emerald-400 text-[#10221c]"
                        : "bg-[#344144] text-slate-200"
                    }`}
                  >
                    {number}
                  </span>
              ...
              {[...drawn]
                ...
                .map((n) => {
                  ...
                  return (
                    <div
                      key={n}
                      className={`aspect-square rounded text-[9px] font-bold flex items-center justify-center ${isHit ? "bg-emerald-600 text-white" : "bg-[#263133] text-slate-500"}`}
                    >
                      {n}
                    </div>
                  );
            ...
            <div className="border-t border-white/10 pt-2">
              ...
              <div className="mt-1.5 flex flex-wrap gap-1">
                {drawn.map((number, index) => (
                  <span
                    key={`${number}-${index}`}
                    className={`flex h-6 min-w-6 items-center justify-center rounded-full px-1 text-[9px] font-black ${ticket.picks.includes(number) ? "bg-emerald-400 text-[#10221c]" : "bg-[#344144] text-slate-300"}`}
                  >
                    {number}
                  </span>
                ))}
...
export default function KenoPage() {
  ...
  const [globalRounds, setGlobalRounds] = useState<
    { roundId: number; drawnNumbers: number[]; drawnAt: string }[]
  >([]);
  const [rightTab, setRightTab] = useState<"results" | "stats" | "leaders">(
    "results",
  );
  ...
  const [placedTickets, setPlacedTickets] = useState<
    { picks: number[]; betAmount: string }[]
  >([]);
  ...
  const [batchResult, setBatchResult] = useState<CompletedRoundResult | null>(
    null,
  );
  ...
  const [completedRoundResult, setCompletedRoundResult] =
    useState<CompletedRoundResult | null>(null);
  ...
  async function loadHistory() {
    try {
      const res = await apiFetch(
        `/api/games/keno/history?mode=${mode}&limit=50`,
      );
  ...
  useEffect(() => {
    ...
    const fetchState = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Silently ignore rate limit — next poll will retry
          return;
        }
      } catch {
    ...
    // Adaptive interval: faster during drawing, slower during betting
    const intervalMs = roundState?.phase === "drawing" ? 1000 : 2000;
  ...
  }, [mode, roundState?.phase, roundState?.roundId]);
  ...
  useEffect(() => {
    ...
    const snapshotResult =
      myBatchSnap &&
      myBatchSnap.totalPayout !== undefined &&
      myBatchSnap.tickets.every((t) => t.hitCount !== undefined)
        ? {
    ...
    if (snapshotResult) {
      ...
      if (currentMode === "real") {
        setWallet((prev) =>
          prev
            ? { ...prev, realBalance: String(myBatchSnap!.newBalance) }
            : prev,
        );
      } else {
        setWallet((prev) =>
          prev
            ? { ...prev, demoBalance: String(myBatchSnap!.newBalance) }
            : prev,
        );
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      roundState.roundId !== lastRoundIdRef.current
    ) {
  ...
  const canPlace =
    ...
    activeBet <= balance &&
    (!settings ||
      (activeBet >= parseFloat(settings.minBet) &&
        activeBet <= parseFloat(settings.maxBet))) &&
  ...
  async function handlePlace() {
    ...
    try {
      ...
      if (!res.ok) {
        toast({
          description: data.error ?? "Bet failed",
          variant: "destructive",
        });
        return;
      }
      ...
      setPlacedTickets((prev) => [
        ...prev,
        { picks: currentPicks, betAmount: activeBet.toFixed(2) },
      ]);
      setCurrentPicks([]); // clear grid so user can pick next ticket
      if (mode === "real") {
        setWallet((prev) =>
          prev
            ? { ...prev, realBalance: String(data.balanceAfterDeduction) }
            : prev,
        );
      } else {
        setWallet((prev) =>
          prev
            ? { ...prev, demoBalance: String(data.balanceAfterDeduction) }
            : prev,
        );
      }
    } catch {
  ...
  async function resetDemo() {
    ...
    try {
      const res = await apiFetch("/api/games/keno/demo/reset", {
        method: "POST",
      });
      if (res.ok) {
        setWallet((prev) =>
          prev ? { ...prev, demoBalance: "10000.00" } : prev,
        );
  ...
  const rtp =
    currentPicks.length > 0 ? calcRtp(currentPicks.length, paytable) : null;
  const frequencyValues = kenoStats ? Object.values(kenoStats.frequency) : [];
  const sortedFrequencies = [...frequencyValues].sort((a, b) => a - b);
  const hotCutoff = sortedFrequencies.length
    ? sortedFrequencies[Math.floor(sortedFrequencies.length * 0.75)]
    : 0;
  const coldCutoff = sortedFrequencies.length
    ? sortedFrequencies[Math.floor(sortedFrequencies.length * 0.25)]
    : 0;
    ...
    <AppLayout showNav={false} wide>
      <div className="min-h-screen bg-[#121719] text-slate-100">
        <header className="flex h-16 items-center justify-between border-b border-white/10 bg-[#171c1d] px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-lg font-black tracking-[0.22em] text-white">
                  XENDRX<span className="text-emerald-400">KENO</span>
                </span>
                {mode === "demo" && (
                  <span className="rounded-full bg-cyan-400/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-cyan-300">
                    Demo
                  </span>
                )}
              </div>
              <p className="hidden text-[10px] uppercase tracking-[0.18em] text-slate-500 sm:block">
                Pick your numbers · play your way
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {mode === "real" && (
              <button
                type="button"
                data-testid="button-topup-keno"
                onClick={() => setShowTopUp(true)}
                className="hidden rounded-lg bg-emerald-500 px-3 py-2 text-xs font-bold text-[#10221c] transition hover:bg-emerald-400 sm:block"
              >
                Top up
              </button>
            )}
            ...
            {mode === "demo" && balance < 1 && (
              <button
                ...
                className="rounded-lg bg-cyan-400 px-3 py-2 text-xs font-bold text-[#10221c] disabled:opacity-50"
              >
                {resettingDemo ? (
            ...
            <button
              type="button"
              data-testid="button-toggle-keno-history"
              onClick={() => {
                setShowHistory((s) => !s);
                setShowRightPanel(false);
              }}
              className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white lg:hidden"
        ...
        <div className="mx-auto grid max-w-[1440px] gap-3 p-3 sm:p-5 lg:grid-cols-[235px_minmax(0,1fr)_280px] lg:gap-4 lg:p-6">
          <aside
            className={`${showHistory ? "block" : "hidden"} rounded-xl border border-white/10 bg-[#1b2324] lg:block`}
          >
            <div className="border-b border-white/10 px-4 py-3">
              ...
              <div className="mt-4 flex items-center justify-between text-[10px] text-slate-500">
                <span>
                  {sidebarTab === "history" ? "Past rounds" : "My tickets"}
                </span>
                ...
                <>
                  {ticketsThisRound === 0 && !animating && (
                    <div className="rounded-lg border border-dashed border-white/10 px-3 py-8 text-center text-xs text-slate-500">
                      Pick numbers and place a bet — your tickets will appear
                      here.
                    </div>
                  )}
                  {animating && (
                    ...
                    <div className="space-y-2">
                      {!animating && !completedRoundResult && (
                        <p className="px-1 text-[10px] font-bold uppercase tracking-wider text-emerald-400">
                          {placedTickets.length} ticket
                          {placedTickets.length !== 1 ? "s" : ""} placed ·
                          waiting for draw…
                        </p>
                      )}
                      {!animating && completedRoundResult && (
                      ...
                      {placedTickets.map((ticket, idx) => (
                        <div
                          key={idx}
                          className="rounded-lg border border-white/5 bg-[#222c2d] p-2.5"
                        >
                          ...
                          <div className="mt-1.5 text-[10px] text-slate-500">
                            {ticket.picks.length} number
                            {ticket.picks.length !== 1 ? "s" : ""} chosen
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    return Array.from(groups.entries()).map(
                      ([key, tickets]) => {
                        ...
                        const totalPayout = tickets.reduce(
                          (s, t) => s + parseFloat(t.payoutAmount),
                          0,
                        );
                        const totalBet = tickets.reduce(
                          (s, t) => s + parseFloat(t.betAmount),
                          0,
                        );
                        ...
                        return (
                          <div
                            key={key}
                            className="rounded-lg border border-white/5 bg-[#222c2d] p-2.5 space-y-2"
                          >
                            <div className="flex items-center justify-between">
                            ...
                            <div className="flex flex-wrap gap-0.5">
                              {first.drawnNumbers.map((n) => (
                                <span
                                  key={n}
                                  className="flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-b from-gray-200 to-gray-400 text-[8px] font-black text-gray-900"
                                >
                                  {n}
                                </span>
                              ))}
                            ...
                            {tickets.map((ticket, ti) => (
                              <div
                                key={ti}
                                className="rounded border border-white/5 bg-[#1b2324] p-1.5"
                              >
                                <div className="mb-1 flex items-center justify-between text-[9px]">
                                  <span className="text-slate-400">
                                    Ticket {ti + 1} · {ticket.betAmount} USDT ·{" "}
                                    {ticket.hitCount} hit
                                    {ticket.hitCount !== 1 ? "s" : ""}
                                  </span>
                                  <span
                                    className={
                                      parseFloat(ticket.payoutAmount) > 0
                                        ? "font-bold text-emerald-400"
                                        : "text-slate-500"
                                    }
                                  >
                                    {parseFloat(ticket.payoutAmount) > 0
                                      ? `+${parseFloat(ticket.payoutAmount).toFixed(2)}`
                                      : "0.00"}{" "}
                                    USDT
                                  </span>
                                </div>
                                <div className="flex flex-wrap gap-0.5">
                                  {ticket.picks.map((n) => (
                                    <span
                                      key={n}
                                      className={`flex h-5 w-5 items-center justify-center rounded-full text-[8px] font-black ${drawn.has(n) ? "bg-yellow-400 text-gray-900" : "bg-slate-700 text-slate-300"}`}
                                    >
                                      {n}
                                    </span>
                                  ))}
                            ...
                            <div className="flex items-center justify-between text-[9px]">
                              ...
                              >
                                Net: {isWin ? "+" : ""}
                                {(totalPayout - totalBet).toFixed(2)} USDT
                              </span>
                            </div>
                          </div>
                        );
            ...
            {mode === "real" && (
              <div className="border-t border-white/10 p-3">
                <button
                  type="button"
                  data-testid="button-withdraw-keno"
                  onClick={() => setShowWithdraw(true)}
                  className="w-full rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-slate-300 transition hover:border-emerald-400/50 hover:text-emerald-300"
                >
                  Withdraw to main wallet
                </button>
              </div>
          ...
          <main className="min-w-0">
            ...
            {roundState?.roundId != null && (
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-emerald-400/20 bg-emerald-400/5 px-2.5 py-1 text-[10px] font-bold text-emerald-300 transition hover:bg-emerald-400/10"
                >
                  <ShieldCheck className="h-3 w-3" />{" "}
                  {roundState.serverSeedRevealed
                    ? "Fairness verified after draw"
                    : "Fairness committed"}
                </button>
              </div>
            )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-300">
                      {completedRoundResult.tickets.length} ticket
                      {completedRoundResult.tickets.length !== 1 ? "s" : ""} ·{" "}
                      {completedRoundResult.tickets.reduce(
                        (sum, ticket) => sum + ticket.hitCount,
                        0,
                      )}{" "}
                      total hit
                      {completedRoundResult.tickets.reduce(
                    ...
                    <p className="mt-2 text-xs font-bold text-white">
                      Updated{" "}
                      {completedRoundResult.mode === "demo" ? "Demo " : ""}
                      Balance: ${completedRoundResult.updatedBalance.toFixed(2)}
                    </p>
                  </div>
            ...
            <div className="mb-3 overflow-hidden rounded-xl border border-white/10 bg-[#273335]">
              ...
              <div className="relative flex min-h-[112px] items-center justify-between overflow-hidden px-5 py-5 sm:px-8">
                ...
                <div className="relative z-20">
                  ...
                  <p className="mt-1 text-sm font-semibold text-cyan-300">
                    ...
                    {(roundState?.totalBets ?? 0) > 0 && !animating && (
                      <span className="ml-2 text-slate-400">
                        · {roundState!.totalBets} player
                        {roundState!.totalBets !== 1 ? "s" : ""} betting
                      </span>
                    )}
                  </p>
                </div>
                ...
                <div className="relative z-20 flex flex-col items-center gap-1">
                  {animating ? (
                    ...
                    <div className="relative flex h-16 w-16 items-center justify-center">
                      ...
                      <span
                        className={`font-mono text-2xl font-black ${countdown <= 5 ? "text-red-400" : countdown <= 10 ? "text-orange-400" : "text-emerald-300"}`}
                      >
                        {countdown}
                      </span>
                    </div>
                ...
                <button
                  type="button"
                  data-testid="button-open-paytable"
                  onClick={() => setShowPaytable(true)}
                  className="absolute right-3 top-3 flex h-7 w-7 items-center justify-center rounded-full bg-cyan-300/10 text-cyan-300 hover:bg-cyan-300/20"
                  aria-label="Open payout information"
                >
            ...
            <div className="rounded-xl border border-white/10 bg-[#1b2324] p-2 sm:p-3">
              ...
              <div className="grid grid-cols-10 gap-1 sm:gap-1.5">
                {(() => {
                  ...
                  const allPlacedPicksSet = new Set(
                    placedTickets.flatMap((t) => t.picks),
                  );
                      ...
                      return (
                        ...
                        >
                          ...
                          {!isSelected && !isDrawn && (isHot || isCold) && (
                            <span
                              className={`absolute right-1 top-1 h-1.5 w-1.5 rounded-full ${isHot ? "bg-red-400" : "bg-sky-400"}`}
                              title={
                                isHot
                                  ? `Hot number · drawn ${frequency} times`
                                  : `Cold number · drawn ${frequency} times`
                              }
                            />
                          )}
                        </button>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3 px-1 text-[9px] text-slate-500">
                ...
                <span className="flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-red-400" /> Hot ·
                  drawn often
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-sky-400" /> Cold
                  · drawn less often
                </span>
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-white/10 pt-3">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    data-testid="button-clear-keno-selection"
                    onClick={() => setCurrentPicks([])}
                    disabled={currentPicks.length === 0 || animating}
                    className="text-xs font-bold text-slate-500 hover:text-white disabled:opacity-30"
                  >
                    Clear
                  </button>
                  <button
                    type="button"
                    data-testid="button-show-payouts"
                    onClick={() => setShowPaytable(true)}
                    disabled={currentPicks.length === 0}
                    className="flex items-center gap-1 text-xs font-bold text-cyan-300 hover:text-cyan-200 disabled:opacity-30"
                  >
                    <Info className="h-3 w-3" /> Payouts
                  </button>
                </div>
              </div>
              ...
              <div className="mt-3 grid grid-cols-[1fr_auto_auto] gap-2">
                <div className="flex items-center rounded-lg border border-white/10 bg-[#263133]">
                  <button
                    ...
                    onClick={() =>
                      setCurrentBetAmount(
                        Math.max(0.1, activeBet - 0.1).toFixed(2),
                      )
                    }
                    disabled={animating}
                    className="flex h-11 w-10 items-center justify-center text-slate-400 hover:text-white disabled:opacity-30"
                  >
                  ...
                  <label className="flex flex-1 items-center justify-center gap-1 text-sm font-black text-white">
                    <input
                      ...
                      step="0.10"
                      disabled={animating}
                      className="w-16 bg-transparent text-center outline-none disabled:opacity-50"
                  ...
                  <button
                    ...
                    onClick={() =>
                      setCurrentBetAmount((activeBet + 0.1).toFixed(2))
                    }
                    disabled={animating}
                    className="flex h-11 w-10 items-center justify-center text-slate-400 hover:text-white disabled:opacity-30"
                  >
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentBetAmount((activeBet * 2).toFixed(2))
                  }
                  disabled={animating}
                  className="rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-3 text-xs font-black text-emerald-300 hover:bg-emerald-400/20 disabled:opacity-30"
                >
                  X2
                </button>
                <button
                  ...
                  onClick={() =>
                    setCurrentBetAmount(settings?.maxBet ?? "100.00")
                  }
                  disabled={animating}
                  className="rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-3 text-xs font-black text-emerald-300 hover:bg-emerald-400/20 disabled:opacity-30"
                >
              ...
              <div className="mt-3">
                ...
                {ticketsThisRound > 0 && !animating && (
                  <div className="mb-2 flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5">
                    ...
                    <span className="text-[11px] font-bold text-emerald-300">
                      {ticketsThisRound} ticket
                      {ticketsThisRound !== 1 ? "s" : ""} placed this round
                    </span>
                ...
                >
                  {animating ? (
                    ...
                    <span className="flex items-center justify-center gap-2">
                      <Check className="h-4 w-4" /> Max tickets reached —
                      waiting for draw
                    </span>
                ...
                <div className="mt-1.5 flex items-center justify-between px-1 text-[10px] text-slate-500">
                  <span>
                    {settings
                      ? `${settings.minBet}–${settings.maxBet} USDT per ticket · up to ${MAX_TICKETS_PER_ROUND} tickets`
                      : `Up to ${MAX_TICKETS_PER_ROUND} tickets per round`}
                  </span>
          ...
          <aside
            className={`${showRightPanel ? "block" : "hidden"} rounded-xl border border-white/10 bg-[#1b2324] lg:block`}
          >
            <div className="border-b border-white/10 px-4 py-3">
              <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-wider">
                <button
                  ...
                  className={`flex items-center gap-1 pb-2 transition-colors ${rightTab === "results" ? "border-b-2 border-emerald-400 text-emerald-300" : "text-slate-500 hover:text-slate-200"}`}
                >
                  <Check className="h-3 w-3" /> Results
                </button>
                <button
                  type="button"
                  ...
                  className={`flex items-center gap-1 pb-2 transition-colors ${rightTab === "stats" ? "border-b-2 border-emerald-400 text-emerald-300" : "text-slate-500 hover:text-slate-200"}`}
                >
                  <BarChart3 className="h-3 w-3" /> Stats
                </button>
                <button
                  type="button"
                  data-testid="button-leaders"
                  onClick={() => {
                    setRightTab("leaders");
                    loadLeaders();
                  }}
                  className={`flex items-center gap-1 pb-2 transition-colors ${rightTab === "leaders" ? "border-b-2 border-emerald-400 text-emerald-300" : "text-slate-500 hover:text-slate-200"}`}
                >
              ...
              {rightTab === "stats" && kenoStats && (
                <p className="mt-3 text-[10px] text-slate-500">
                  Based on last {kenoStats.totalRounds} draws · hot = drawn most
                  often
                </p>
              )}
              {rightTab === "leaders" && (
                <p className="mt-3 text-[10px] text-slate-500">
                  Top 10 players by net profit (real mode)
                </p>
              )}
            </div>
              ...
              {rightTab === "results" && (
                <>
                  ...
                  {globalRounds.map((round, idx) => (
                    <div
                      key={idx}
                      data-testid={`row-keno-result-${idx}`}
                      className="mb-1 rounded-lg bg-[#273335] p-2"
                    >
                      <div className="flex items-center justify-between text-[10px]">
                        <span className="flex items-center gap-1 font-semibold text-emerald-300">
                          <Check className="h-3 w-3" /> Draw #{round.roundId}
                        </span>
                        <span className="text-slate-500">
                          {new Date(round.drawnAt).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
                      ...
                      <div className="mt-1 grid grid-cols-5 gap-1">
                        {round.drawnNumbers.slice(0, 10).map((number) => (
                          <span
                            key={number}
                            className="rounded bg-[#344144] py-0.5 text-center text-[9px] font-bold text-slate-400"
                          >
                            {number}
                          </span>
                        ))}
                              ...
                              (n) => {
                                ...
                                return (
                                  <div
                                    key={n}
                                    title={`#${n}: drawn ${freq}×`}
                                    className={`flex aspect-square items-center justify-center rounded text-[8px] font-bold ${bg}`}
                                  >
                                    {n}
                                  </div>
                                );
              ...
              {rightTab === "leaders" && (
                <>
                  {kenoLeaders.length === 0 && (
                    <p className="px-3 py-8 text-center text-xs text-slate-500">
                      No real-money games played yet.
                    </p>
                  )}
                  {kenoLeaders.length > 0 && (
                    <div className="space-y-1 p-1">
                      {kenoLeaders.map((leader, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 rounded-lg bg-[#273335] px-3 py-2"
                        >
                          <span
                            className={`w-5 text-center text-[10px] font-black ${idx === 0 ? "text-yellow-400" : idx === 1 ? "text-slate-300" : idx === 2 ? "text-amber-600" : "text-slate-500"}`}
                          >
                            {idx === 0
                              ? "🥇"
                              : idx === 1
                          ...
                          <span className="flex-1 truncate text-[10px] font-semibold text-slate-200">
                            {leader.username}
                          </span>
                          <span
                            className={`text-[10px] font-bold ${parseFloat(leader.netProfit) >= 0 ? "text-emerald-400" : "text-red-400"}`}
                          >
                            {parseFloat(leader.netProfit) >= 0 ? "+" : ""}
                            {parseFloat(leader.netProfit).toFixed(2)} USDT
                          </span>
                        </div>
                      ))}
                    </div>
